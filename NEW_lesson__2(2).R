...
#Здесь же познакомимся с пакетом lubridate, предназначенного для работы с датами
...
f.1<-function(d){
  t<-sum(datn$price[day(ymd(datn$d.date))==d]) #здесь необходимо добавить return(t)
  return(t)
   }
...
data()
head(Orange,17) # окружность измерялась в одинаковом возрасте
?Orange
Orange
str(Orange)
...
read.csv("cardio_train.csv",sep=";",nrow=5,row.names =1)# в качестве имен строк можно и столбец вязть
